...
//   mpx from-image  <input> [filter] <output.mpx>
//   mpx to-image    <input.mpx> <output>
//   mpx view        <input.mpx>
//   mpx info        <input.mpx>
//   mpx bench       <input.mpx>
...
use mpx::{decode_image, encode_image, ColorType, FilterType, MpxHeader};
...
fn main() {
    ...
    match args[1].as_str() {
        ...
        "from-image" => cmd_from_image(&args),
        "to-image" => cmd_to_image(&args),
        "view" => cmd_view(&args),
        "info" => cmd_info(&args),
...
fn cmd_from_image(args: &[String]) {
    // mpx from-image <input> [filter] <output.mpx>
    // Supports any format the `image` crate understands: PNG, JPEG, BMP,
    // GIF, TIFF, WebP, …  Output format is always MPX.
    if args.len() < 4 {
        usage();
        process::exit(1);
    }
    ...
    let img = image::open(src_path)
        .unwrap_or_else(|e| die(&format!("open '{}': {}", src_path, e)));

    let (w, h) = (img.width(), img.height());
    let color = img.color();

    let (mpx_ct, bpp, raw): (ColorType, u8, Vec<u8>) = match color {
        image::ColorType::L8 => (ColorType::Gray, 8, img.into_luma8().into_raw()),
        image::ColorType::La8 => (ColorType::GrayA, 8, img.into_luma_alpha8().into_raw()),
        image::ColorType::Rgb8 => (ColorType::Rgb, 8, img.into_rgb8().into_raw()),
        image::ColorType::Rgba8 => (ColorType::Rgba, 8, img.into_rgba8().into_raw()),
        image::ColorType::L16 => {
            let raw: Vec<u8> = img
                .into_luma16()
                .into_raw()
        ...
        other => {
            eprintln!(
                "Note: unsupported color type {:?} — converting to Rgb8",
                other
            );
            let converted = image::open(src_path).unwrap().into_rgb8();
            (ColorType::Rgb, 8, converted.into_raw())
        }
    };
    ...
    println!("from-image: {} → {}", src_path, out_path);
    println!(
        "  {} × {} {:?} {}bpp  [filter={:?}]",
...
fn cmd_to_image(args: &[String]) {
    // mpx to-image <input.mpx> <output>
    // Output format is inferred from the file extension (PNG, JPEG, BMP, …).
    if args.len() < 4 {
        usage();
        process::exit(1);
    }
    ...
    dynimg
        .save(out_path)
        .unwrap_or_else(|e| die(&format!("save '{}': {}", out_path, e)));

    println!("to-image: {} → {}", mpx_path, out_path);
    println!(
        "  {} × {} {:?} {}bpp  decoded in {:.2}ms",
        header.width,
        header.height,
...
fn cmd_view(args: &[String]) {
    ...
    // Reuse to-image logic
    let to_image_args = vec![
        String::new(),
        "to-image".into(),
        mpx_path.clone(),
        tmp_str.clone(),
    ];
    cmd_to_image(&to_image_args);
...
fn cmd_roundtrip(args: &[String]) {
    ...
    let encoded =
        encode_image(width, height, color_type, bit_depth, FilterType::Paeth, &raw)
            .unwrap_or_else(|e| die(&format!("encode: {}", e)));

    let (_, decoded) =
        decode_image(&encoded).unwrap_or_else(|e| die(&format!("decode: {}", e)));
    ...
    if decoded == raw {
        ...
        for (i, (a, b)) in raw.iter().zip(decoded.iter()).enumerate() {
            if a != b {
                eprintln!(
                    "  First diff at byte {}: original={} decoded={}",
                    i, a, b
                );
                break;
            }
...
fn pixels_to_dynimage(header: &MpxHeader, pixels: Vec<u8>) -> DynamicImage {
    ...
    match (header.color_type, header.bit_depth) {
        ...
        (ct, bpp) => die(&format!(
            "unsupported {:?} {}bpp for image output",
            ct, bpp
        )),
    }
}
...
fn parse_color_type(s: &str) -> ColorType {
    match s.to_lowercase().as_str() {
        ...
        "rgba" => ColorType::Rgba,
        _ => die(&format!(
            "unknown color type '{}' (gray | graya | rgb | rgba)",
            s
        )),
    }
}
...
fn usage() {
    ...
    eprintln!("  mpx from-image  <in> [filter] <out.mpx>   (PNG/JPEG/BMP/TIFF/WebP/…)");
    eprintln!("  mpx to-image    <in.mpx> <out>             (format from extension)");
    eprintln!("  mpx view        <in.mpx>                   (decode + open system viewer)");
